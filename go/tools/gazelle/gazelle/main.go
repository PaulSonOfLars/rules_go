...
var (
	buildFileName = flag.String("build_file_name", "BUILD,BUILD.bazel", "comma-separated list of valid build file names, the first element of this list is the name of output build files to generate.")
...
func run(dirs []string, emit func(*bzl.File) error, external rules.ExternalResolver) error {
	g, err := generator.New(*repoRoot, *goPrefix, *prefixRoot, getBuildFileName(), *buildTags, external)
	if err != nil {
		return err
	}
